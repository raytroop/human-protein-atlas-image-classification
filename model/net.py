...
def densenet121(pretrained=False, state_dict=None, **kwargs):
    r"""Densenet-121 model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`_

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        state_dict (OrderedDict): pretrained model state dict
        num_classes (int): # of class in final classifier layer
    """
...
class DenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #     features.size(0), -1)
        # adaptive `kernel_size` instead of hard-coding
        out = F.avg_pool2d(out, kernel_size=features.size(2), stride=1).view(
            features.size(0), -1)
