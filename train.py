...
if __name__ == '__main__':
    ...
    Net = getattr(net, params.model)
    if params.model == 'densenet121':
        state_dict = torch.load(os.path.expanduser('~/.torch/models/densenet121-a639ec97.pth'))
    model = Net(params.pretrained, state_dict=state_dict, num_classes=28).cuda()\
            if params.cuda else Net(params.pretrained, state_dict=state_dict, num_classes=28)
    # pylint: enable=E1101
    ...
    optimizer = optim.Adam(
        [{'params': model.features.parameters(), 'lr': (params.learning_rate)/10},
